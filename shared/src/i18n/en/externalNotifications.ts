...
const en: NotificationLocale = {
  email: {
    footer: 'You received this because you have notifications enabled in Savanna Escape.',
    manage: 'Manage preferences in Settings',
    madeWith: 'Made with',
    openTrek: 'Open Savanna Escape',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} invited you to fuse vacation plans. Open Savanna Escape to accept or decline.`,
    }),
    collection_invite: (p) => ({
      title: 'Collection invite',
      body: `${p.actor} invited you to share a collection. Open Savanna Escape to accept or decline.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'New Savanna Escape version available',
      body: `Savanna Escape ${p.version} is now available. Visit the admin panel to update.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Hi',
    body: 'We received a request to reset the password for your Savanna Escape account. Click the button below to set a new password.',
